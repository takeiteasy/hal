...
static const ASensor *gravity_sensor = NULL;
...
static int sensor_callback(int fd, int events, void *data) {
    ...
    while (ASensorEventQueue_getEvents(sensor_event_queue, &event, 1) > 0) {
        if (event.type == ASENSOR_TYPE_GRAVITY) {
            sensor_data.x = event.vector.x;
            sensor_data.y = event.vector.y;
            sensor_data.z = event.vector.z;
...
static void init_sensor_manager(void) {
    ...
    if (sensor_manager)
        gravity_sensor = ASensorManager_getDefaultSensor(sensor_manager, ASENSOR_TYPE_GRAVITY);
}

bool hal_gravity_available(void) {
    init_sensor_manager();
    return gravity_sensor != NULL;
}

void hal_gravity_enable(void) {
    if (is_enabled) return;
    init_sensor_manager();
    if (!gravity_sensor) return;
    ...
    if (ASensorEventQueue_enableSensor(sensor_event_queue, gravity_sensor) < 0) {
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
        return;
    }
    ASensorEventQueue_setEventRate(sensor_event_queue, gravity_sensor, 1000000 / 60);
    is_enabled = true;
}
...
void hal_gravity_disable(void) {
    ...
    if (sensor_event_queue) {
        ASensorEventQueue_disableSensor(sensor_event_queue, gravity_sensor);
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
    }
    is_enabled = false;
