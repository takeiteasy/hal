...
static const ASensor *humidity_sensor = NULL;
...
static float humidity_value = -1.0f;
...
static int sensor_callback(int fd, int events, void *data) {
    ...
    while (ASensorEventQueue_getEvents(sensor_event_queue, &event, 1) > 0) {
        if (event.type == ASENSOR_TYPE_RELATIVE_HUMIDITY) {
...
static void init_sensor_manager(void) {
    ...
    if (sensor_manager)
        humidity_sensor = ASensorManager_getDefaultSensor(sensor_manager, ASENSOR_TYPE_RELATIVE_HUMIDITY);
}

bool hal_humidity_available(void) {
    init_sensor_manager();
    return humidity_sensor != NULL;
}
...
void hal_humidity_enable(void) {
    ...
    if (ASensorEventQueue_enableSensor(sensor_event_queue, humidity_sensor) < 0) {
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
        return;
    }
    ASensorEventQueue_setEventRate(sensor_event_queue, humidity_sensor, 1000000 / 5); // 5Hz
    is_enabled = true;
}
...
void hal_humidity_disable(void) {
    ...
    if (sensor_event_queue) {
        ASensorEventQueue_disableSensor(sensor_event_queue, humidity_sensor);
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
    }
    is_enabled = false;
...
bool hal_humidity_enabled(void) { return is_enabled; }

float hal_humidity_get(void) {
    pthread_mutex_lock(&sensor_mutex);
    float result = humidity_valid ? humidity_value : -1.0f;
    pthread_mutex_unlock(&sensor_mutex);
    return result;
}
#endif
