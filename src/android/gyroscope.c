...
static void init_sensor_manager(void) {
    ...
    if (sensor_manager)
        gyro_sensor = ASensorManager_getDefaultSensor(sensor_manager, ASENSOR_TYPE_GYROSCOPE);
}

bool hal_gyroscope_available(void) {
    init_sensor_manager();
    return gyro_sensor != NULL;
}

void hal_gyroscope_enable(void) {
    if (is_enabled) return;
    init_sensor_manager();
    if (!gyro_sensor) return;
    ...
    if (ASensorEventQueue_enableSensor(sensor_event_queue, gyro_sensor) < 0) {
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
        return;
    }
    ASensorEventQueue_setEventRate(sensor_event_queue, gyro_sensor, 1000000 / 60);
    is_enabled = true;
}
...
void hal_gyroscope_disable(void) {
    ...
    if (sensor_event_queue) {
        ASensorEventQueue_disableSensor(sensor_event_queue, gyro_sensor);
        ASensorManager_destroyEventQueue(sensor_manager, sensor_event_queue);
        sensor_event_queue = NULL;
    }
    is_enabled = false;
